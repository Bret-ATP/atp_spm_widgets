// ===========================================
// Strategic Objectives Widget Pack
// Shared client-side EntityState filtering across Strategic Objective widgets.
// v5: Restores modal cleanup and adds an 8px left inset so entity content is not clipped.
// ===========================================
(function () {
    var TP_OKR_SCOPE = 'StrategicObjective';
    var TP_OKR_EVENT = 'tpOkrStrategicObjectiveFilterChanged';
    ...
    function getSharedStateIds() {
        return (window.tpOkrSharedFilters[TP_OKR_SCOPE].selectedStateIds || []).slice();
    }
    ...
    function setSharedStateIds(ids, sourceId) {
        ...
        window.dispatchEvent(new CustomEvent(TP_OKR_EVENT, {
            detail: {
                scope: TP_OKR_SCOPE,
                sourceId: sourceId || null,
                selectedStateIds: normalized
            }
        }));
    }
    ...
    function getEntityState(obj) {
        ...
        if (!state) {
            return { id: '__none__', name: 'No State' };
        }
        return {
            id: state.Id !== undefined && state.Id !== null ? String(state.Id) : '__none__',
            name: state.Name || 'No State'
        };
    }
    ...
    function filterByState(items, selectedStateIds) {
        ...
        var selected = {};
        selectedStateIds.forEach(function (id) {
            selected[String(id)] = true;
        });
        return (items || []).filter(function (item) {
            return selected[String(item.stateId)] === true;
        });
    }
    ...
    function renderStateFilterBar($parent, states, selectedStateIds, onChange) {
        ...
        $bar.append($('<span>').text('State:').css({
            'font-weight': '700',
            color: '#374151',
            'font-size': '13px',
            'margin-right': '2px'
        ...
        $bar.append($('<span>').text('Total: ' + totalCount).css({
            color: '#6b7280',
            'font-size': '12px',
            'font-weight': '600',
            'margin-right': '6px'
        ...
        $bar.append(makePill('All', !hasActiveFilter, function () {
            onChange([]);
        }));
    ...
    function openEntityModal(entityPage, entityId, entityTitle) {
        ...
        $header.append($('<div>').text(entityTitle || ('Entity #' + entityId)).css({
            'font-weight': '600',
            color: '#111827',
            'font-size': '14px',
            'white-space': 'nowrap',
...
'));
                ...
                for (var i = 0; i < sidePanes.length; i++) {
                    sidePanes[i].style.display = 'none';
                    sidePanes[i].style.visibility = 'hidden';
                    sidePanes[i].style.width = '0';
                    sidePanes[i].style.minWidth = '0';
                    sidePanes[i].style.maxWidth = '0';
                    sidePanes[i].style.overflow = 'hidden';
                }
                ...
                for (var j = 0; j < mainPanes.length; j++) {
                    mainPanes[j].style.left = '0';
                    mainPanes[j].style.marginLeft = '0';
                    mainPanes[j].style.paddingLeft = '8px';
                    mainPanes[j].style.boxSizing = 'border-box';
                    mainPanes[j].style.width = 'calc(100% - 8px)';
                    mainPanes[j].style.maxWidth = 'none';
                }
        ...
        $overlay.append($modal).on('click', function (e) {
    ...
    function registerSharedFilterListener(widgetId, applyFn) {
        window.addEventListener(TP_OKR_EVENT, function (event) {
            if (!event.detail || event.detail.scope !== TP_OKR_SCOPE || event.detail.sourceId === widgetId) {
                return;
            }
            applyFn((event.detail.selectedStateIds || []).map(function (id) {
                return String(id);
            }));
        });
    }

    function loadStrategicObjectives(includeCustomFields, done, fail) {
        var include = includeCustomFields ? '[Id,Name,Progress,EntityState,CustomFields]' : '[Id,Name,Progress,EntityState]';
        $.ajax({
            url: '/api/v1/StrategicObjectives?include=' + include + '&format=json',
            type: 'GET',
            dataType: 'json'
    ...
    function renderObjectiveCard(obj, includeStateLabel) {
        ...
        }).attr('title', 'Click to open Strategic Objective')
        .hover(
            function () {
                $(this).css({
        ...
        if (includeStateLabel) {
            $card.append($('<div>').text(obj.stateName).css({
                color: '#6b7280',
                'font-size': '11px',
                'margin-top': '4px'
            }));
    ...
    function renderKrRow($card, kr) {
        ...
        var $kr = $('<div>').css({
            'margin-top': '12px',
            cursor: 'pointer'
        }).attr('title', 'Click to open Key Result')
        .on('click', function (e) {
            e.stopPropagation();
        ...
        $top.append($('<div>').text(kr.Name).css({
            color: '#374151',
            'font-weight': '600'
        }));

        $top.append($('<div>').text(progress + '%').css({
            color: rag.color,
            'font-weight': '700'
        }));
        ...
        if (detail.length) {
            $kr.append($('<div>').text(detail.join('  ')).css({
                color: '#6b7280',
                'font-size': '11px',
                'margin-top': '2px'
            }));
    ...
    function registerStatusWidget($, readyConfigurator) {
        readyConfigurator.getDashboardWidgetTemplateRegistry().addWidgetTemplate({
            ...
            insert: function (placeholder, settings) {
                ...
                var widgetId = 'so-status-' + Math.random().toString(36).slice(2);
                var allObjectives = [];
                ...
                function renderGauge() {
                    ...
                    objectives.forEach(function (obj) {
                        if (obj.progress >= 70) green++;
                        else if (obj.progress >= 40) amber++;
                        else red++;
                    });
                    ...
                    var selectionText = activeStateIds.length > 0
                        ? (total + ' of ' + fullTotal + ' Strategic Objectives')
                    ...
                    renderStateFilterBar($widget, buildStateCounts(allObjectives), activeStateIds, function (nextStateIds) {
                        activeStateIds = nextStateIds;
                        setSharedStateIds(activeStateIds, widgetId);
                        renderGauge();
                    });
                    ...
                    $center.append($('<div>').text(percentage + '%').css({
                        'font-size': '64px',
                        'font-weight': 'bold',
                        color: status.color,
                        'line-height': '1'
                    ...
                    $center.append($('<div>').text(status.status).css({
                        'font-size': '18px',
                        color: status.color,
                        'margin-top': '8px'
                    }));

                    $center.append($('<div>').text(selectionText).css({
                        'font-size': '13px',
                        color: '#6b7280',
                        'font-weight': '600',
                        'margin-top': '8px'
                    ...
                    $bar.append($('<div>').css({
                        width: (total ? (red / total * 100) : 0) + '%',
                        background: '#ef4444'
                    }));
                    $bar.append($('<div>').css({
                        width: (total ? (amber / total * 100) : 0) + '%',
                        background: '#f59e0b'
                    }));
                    $bar.append($('<div>').css({
                        width: (total ? (green / total * 100) : 0) + '%',
                        background: '#22c55e'
                    }));
                ...
                function render() {
                    loadStrategicObjectives(false, function (items) {
                        allObjectives = items;
                        renderGauge();
                    });
                }

                registerSharedFilterListener(widgetId, function (ids) {
                    activeStateIds = ids;
                    renderGauge();
                });
    ...
    function registerSummaryWidget($, readyConfigurator) {
        readyConfigurator.getDashboardWidgetTemplateRegistry().addWidgetTemplate({
            ...
            insert: function (placeholder, settings) {
                var props = { placeholder: placeholder, settings: settings || {} };
                var widgetId = 'so-summary-' + Math.random().toString(36).slice(2);
                var allObjectives = [];
                var activeStateIds = getSharedStateIds();
                ...
                function renderGrid() {
                    ...
                    if (objectives.length === 0) {
                        $widget.append($('<div>').text('No Strategic Objectives match the selected state filter.').css({
                            color: '#6b7280',
                            padding: '16px 0'
                        }));
                        $container.append($widget);
                    ...
                    objectives.forEach(function (obj) {
                        $grid.append(renderObjectiveCard(obj, true));
                    });
    ...
    function registerOkrWidget($, readyConfigurator) {
        readyConfigurator.getDashboardWidgetTemplateRegistry().addWidgetTemplate({
            ...
            insert: function (placeholder, settings) {
                var props = { placeholder: placeholder, settings: settings || {} };
                var widgetId = 'so-okr-' + Math.random().toString(36).slice(2);
                var allObjectives = [];
                var activeStateIds = getSharedStateIds();
                ...
                function renderGrid() {
                    ...
                    objectives.forEach(function (obj) {
                        ...
                        if (!obj.keyResults || obj.keyResults.length === 0) {
                            $card.append($('<div>').text('No Key Results').css({
                                color: '#9ca3af',
                                'font-size': '12px',
                                'margin-top': '10px'
                            }));
                ...
                function render() {
                    loadStrategicObjectives(true, function (items) {
                        allObjectives = items;
                        renderGrid();
                        loadKeyResultsForObjectives(allObjectives, renderGrid);
                    });
                }
