// ===========================================
// Tactical Objectives Widget Pack
// Shared client-side EntityState filtering across Tactical Objective widgets.
// Includes modal iframe cleanup with an 8px left inset so entity content is not clipped.
// ===========================================
(function () {
    var TP_OKR_SCOPE = 'TacticalObjective';
    var TP_OKR_EVENT = 'tpOkrTacticalObjectiveFilterChanged';
...
'));
    ...
    function loadObjectives(includeCustomFields, done, fail) {
        ...
        $.ajax({ url: '/api/v1/TacticalObjectives?include=' + include + '&format=json', type: 'GET', dataType: 'json' }).done(function (response) {
    ...
    function renderObjectiveCard(obj, includeStateLabel) {
        var $card = $('<div>').css({ padding: '15px', background: '#f9fafb', border: '1px solid #e5e7eb', 'border-radius': '8px', cursor: 'pointer', transition: 'all 0.15s ease' }).attr('title', 'Click to open Tactical Objective').hover(function () { $(this).css({ transform: 'translateY(-2px)', 'box-shadow': '0 6px 16px rgba(0,0,0,0.1)' }); }, function () { $(this).css({ transform: 'none', 'box-shadow': 'none' }); }).on('click', function () { openEntityModal('tacticalobjective', obj.id, obj.name); });
    ...
    function registerStatusWidget($, readyConfigurator) {
        readyConfigurator.getDashboardWidgetTemplateRegistry().addWidgetTemplate({ id: 'to_status_color_bar_widget', name: 'Tactical Objectives Status', previewSrc: 'https://raw.githubusercontent.com/Bret-ATP/atp_spm_widgets/main/img/so_status_preview.png', description: 'Status summary of Tactical Objectives', tags: ['OKR', 'Tactical Objectives'], placeholder: 'restui_board', insert: function (placeholder, settings) {
            var props = { placeholder: placeholder, settings: settings || {} };
            var widgetId = 'to-status-' + Math.random().toString(36).slice(2);
            var allObjectives = [];
            ...
            function renderGauge() {
                ...
                var selectionText = activeStateIds.length > 0 ? (total + ' of ' + fullTotal + ' Tactical Objectives') : (total + ' Tactical Objectives');
    ...
    function registerSummaryWidget($, readyConfigurator) {
        readyConfigurator.getDashboardWidgetTemplateRegistry().addWidgetTemplate({ id: 'tactical_objectives_summary_progress', name: 'Tactical Objectives Summary', previewSrc: 'https://raw.githubusercontent.com/Bret-ATP/atp_spm_widgets/1dfb4b32efe1a64da9f918c7a114619fcc903d35/img/so_status_summary_preview.png', description: 'Tactical Objectives grid with progress and status', tags: ['OKR', 'Tactical Objectives'], placeholder: 'restui_board', insert: function (placeholder, settings) {
            var props = { placeholder: placeholder, settings: settings || {} };
            var widgetId = 'to-summary-' + Math.random().toString(36).slice(2);
            var allObjectives = [];
            var activeStateIds = getSharedStateIds();
            function renderGrid() {
                var $container = $(props.placeholder).empty();
                ...
                var objectives = filterByState(allObjectives, activeStateIds);
                if (objectives.length === 0) { $widget.append($('<div>').text('No Tactical Objectives match the selected state filter.').css({ color: '#6b7280', padding: '16px 0' })); $container.append($widget); return; }
    ...
    function registerOkrWidget($, readyConfigurator) {
        readyConfigurator.getDashboardWidgetTemplateRegistry().addWidgetTemplate({ id: 'tactical_objectives_grid_krs', name: 'Tactical OKR Summary', previewSrc: 'https://raw.githubusercontent.com/Bret-ATP/atp_spm_widgets/37891ae2093ff8d61637ded01e2872294e3c990a/img/so_status_summary_kr_preview.png', description: 'Tactical Objectives with Key Results', tags: ['OKR', 'Tactical Objectives'], placeholder: 'restui_board', insert: function (placeholder, settings) {
            var props = { placeholder: placeholder, settings: settings || {} };
            var widgetId = 'to-okr-' + Math.random().toString(36).slice(2);
            var allObjectives = [];
            var activeStateIds = getSharedStateIds();
            function renderGrid() {
                var $container = $(props.placeholder).empty();
            ...
            function loadKeyResultsForObjectives(objectives, done) {
                ...
                objectives.forEach(function (obj) {
                    $.ajax({ url: '/api/v1/TacticalObjectives/' + obj.id + '/KeyResults?include=[Id,Name,Progress,CustomFields]&format=json', type: 'GET', dataType: 'json' }).done(function (response) { obj.keyResults = response.Items || []; }).fail(function () { obj.keyResults = []; }).always(function () { remaining--; if (remaining === 0) done(); });
                });
            }
